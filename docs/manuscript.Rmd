...
I generated data with a dichotomous outcome. The average class imbalance was 97% in the negative and 3% in the positive clases. These data were simulated by adapting the `twoClassSim` function from the `caret` R package (Kuhn, 2008). I generated 500 data sets using the following steps:  
...
First, this illustrates the relative importance of sampling techniques over the algorithm used. Doing no sampling technique yielded $> .96$ models making no preeictions for all algorithms—save for AdaBoost, which was able to reduce this to $.23$.  
